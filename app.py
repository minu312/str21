...
def setup_db_route():
    try:
        init_db()
        return "Database configured successfully! <a href='/'>Go to Home</a>"
    except Exception as e:
        import traceback
        return f"<h3>Database Error:</h3><pre>{traceback.format_exc()}</pre>"
...
def index():
    ...
    try:
        ...
        import traceback
        return f"<h3>Homepage Error:</h3><pre>{traceback.format_exc()}</pre>"
...
# Initialize the database so Heroku creates the tables when the app starts
try:
    init_db()
except Exception as e:
